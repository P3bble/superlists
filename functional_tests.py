...
class NewVisitorTest(unittest.TestCase):
    ...
    def test_can_start_a_list_and_retrieve_it_later(self):
        ...
        # is she tying fly-fishing lures)
        ...
        print("self.browser.page_source".upper() + self.browser.page_source) 
        table = self.browser.find_element_by_id('id_list_table')
        rows = table.find_elements_by_tag_name('tr')
        self.assertIn('Buy peacock feathers', [row.text for row in rows])

        # There is still a text box inviting here to add another item. She
